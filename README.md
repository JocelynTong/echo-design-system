...
这套系统让 PM 描述需求、AI 直接生成可预览的 HTML 原型、同步到 Figma 供设计师精调、精调结果回写仓库——开发拿到的是始终与设计保持一致的还原基准，而不是一份随时可能过期的标注图。

**它的意义在于**：消除设计到开发之间的信息损耗。每一次需求交付，原型、设计稿、代码基准三者同源，不再靠人工比对和口头对齐。

涉及四个角色：**PM**（提需求）、**AI**（生成原型）、**设计师**（Figma 精调）、**开发**（按原型还原）。仓库是唯一交付中心。
