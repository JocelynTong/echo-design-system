...
|---|---|---|
| 回响 AI 设计系统 | `05 figma-plugin/manifest.json` | 建稿 + 导出结构 + 查找组件 Key |

导入后在 Figma 里通过「Plugins → Development → 插件名」运行。

---

**步骤一：建稿**（插件：回响 AI 设计系统）
...
config 已复制到剪贴板。打开 Figma → 运行「回响 AI 设计系统」插件 → 粘贴 JSON → 点击「建稿」，自动生成页面帧 + 跳转连线。文件夹里的 HTML 原型可同步对照参考。
...
├── 截图                                   # 快捷脚本：python3 generate.py 截图（可加 --force）
├── 同步                                   # 快捷脚本：python3 generate.py（可加 APP 名）
...
│
├── 02 components/                         # 原子组件规范文档（AI 读，20 个组件）
