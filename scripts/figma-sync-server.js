/**
 * 千岛 Figma Sync
 * 运行: npm run figma-sync <需求文件夹名>
 * 例如: npm run figma-sync community-default
 *
...
try {
  ...
  if (!folder) {
    console.error('❌ 请指定需求文件夹名，例如: npm run figma-sync community-default');
    process.exit(1);
  }
  const configPath = path.join(__dirname, '../demos', folder, 'figma-config.json');
  ...
  config.pages.forEach(p => {
    console.log(`  📄 ${p.name}：${p.components.length} 个组件`);
  });
